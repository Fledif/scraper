...
async def fetch_with_playwright(url, source_name):
...
async def fetch_page_curl(base_url, search_url):
    async with AsyncSession(impersonate="chrome120") as session:
        ...
        if res.status_code in [403, 503] or "just a moment" in lower_text or "cloudflare" in lower_text or "captcha" in lower_text or "checking your browser" in lower_text:
...
async def scrape_work_live(query):
    ...
    try:
        ...
        if not cards:
            raise ValueError("Cloudflare block detected via empty content")
            
        print("[Work.ua] curl_cffi успішно зібрав вакансії")
    except Exception as e:
        html = await fetch_with_playwright(search_url, "Work.ua")
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        cards = soup.find_all('div', class_=lambda x: x and 'card' in x and 'job-link' in x)
...
async def scrape_jobs_live(query):
    ...
    try:
        ...
        if not cards:
            raise ValueError("Cloudflare block detected via empty content")
        print("[Jobs.ua] curl_cffi успішно зібрав вакансії")
    except Exception as e:
        html = await fetch_with_playwright(search_url, "Jobs.ua")
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        cards = soup.find_all('li', class_='b-vacancy__item')
...
async def scrape_jooble_live(query):
    ...
    try:
        ...
        if not cards:
            raise ValueError("Cloudflare block detected via empty content")
        print("[Jooble] curl_cffi успішно зібрав вакансії")
    except Exception as e:
        html = await fetch_with_playwright(search_url, "Jooble")
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        cards = soup.find_all('article') or soup.find_all('div', attrs={"data-test-id": "vacancy-card"})
...
async def scrape_robota_live(query):
    ...
    try:
        ...
        if not cards:
            raise ValueError("Cloudflare block detected via empty content")
        print("[Robota.ua] curl_cffi успішно зібрав вакансії")
    except Exception as e:
        html = await fetch_with_playwright(search_url, "Robota.ua")
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        cards = soup.find_all('article')
        if not cards:
...
async def safe_scrape(task_coro, timeout=12.0):
    try:
        # Збільшено до 12 сек для Playwright
